...
*)
  ...
  Context `{CCM flowdom} `{Countable Node}.
...
Proof.
  ...
  - (* True *)
    ...
    * destruct Ir.
      ...
      assert (k ∉ dom outR0).
      set_solver.
      rewrite nzmap_elem_of_dom_total in H2.
      by apply dec_stable in H2.
      unfold dom, flowint_dom, I_empty, I_emptyR in H1. 
      simpl in H1.
...
Proof.
  ...
  - set_unfold.
    ...
    + intros H2 H1_dec.
      ...
      split.
      apply or_intror.
      intros H_or; destruct H_or as [H_false | H3].
      contradict H_false.
      exact is_Some_None.
      exact H3.
    + intros H2 H1_dec.
      ...
      * intros H3 H2_dec.
        contradict H_valid.
        unfold op, intComp.
        rewrite H1. rewrite H1_dec. rewrite H2_dec.
...
Proof.
  ...
  - (* if composable *)
    ...
    + (* outR equality *)
      ...
      try (rewrite H1 in e *; naive_solver).
      try (rewrite H1 in n *; naive_solver).
      by rewrite ccm_comm.
  - (* if not composable *)
...
Proof.
  ...
  - assert (dom I1 ≡ ∅).
    ...
    rewrite dom_empty.
    reflexivity.
    pose proof (intEmp_unique _ V1 H1).
    trivial.
Qed.
...
Proof.
  ...
  - intros.
    ...
    destruct V1 as (_ & E1).
    destruct V2 as (_ & E2).
    unfold dom, flowint_dom, inf_map in H3, H4.
    apply dom_empty_inv in H3.
    apply dom_empty_inv in H4.
    apply E1 in H3.
    apply E2 in H4.
    unfold out_map.
    rewrite H3.
...
Proof.
  ...
  assert (IC := H0).
  unfold intComposable in H1.
  destruct H1 as (I1v & I2v & Disjoint & I1inf & I2inf).
  unfold map_Forall in I1inf.
  pose proof (I1inf n (inf I1 n)).
  unfold inf in H1.
  rewrite D in H1.
  unfold default, id in H1.
  ...
  - intros Ir Idef. rewrite Idef in V.
    ...
    + unfold inf.
      ...
      * rewrite D.
        unfold default, id.
        rewrite ccm_comm in H2.
        trivial.
      * try done.  
    + unfold not, intComposable in n0.
      exfalso.
      apply n0. split; auto.
  - intros.
    rewrite H3 in V.
...
Proof.
  intros.
  apply intComp_dom in H1 as D.
  rewrite D in H2.
  pose proof (outComp_inv I1 I2 n H2).
  apply intComposable_valid in H1.
...
Proof.
  ...
  { unfold intComposable.
    ...
    - unfold map_Forall.
      ...
      apply mk_is_Some in xDef as H2.
      apply H1 in H2.
      rewrite H2.
      rewrite (ccm_comm _ (inf I n)).
      rewrite ccm_pinv.
      unfold inf at 1 in H2.
      rewrite xDef in H2.
      simpl in H2.
      rewrite H2.
      reflexivity.
  ...
  - apply map_eq.
    ...
    + intros.
      ...
      * intros.
        ...
        assert (n ∉ dom (int Ir)).
        set_solver.
        unfold dom, flowint_dom, inf_map in H6.
        rewrite elem_of_dom in H6 *.
        intros.
        rewrite H1 in H6.
        simpl in H6.
        rewrite <- eq_None_not_Some in H6.
        trivial.
    + try done.     
...
Proof.
  ...
  { apply intComp_fold.
    ...
    - intros ? n_in_I1.
      ...
      rewrite D23.
      set_solver.
      apply intComp_unfold_out in H2.
      unfold inf at 1.
      rewrite n_inf.
      simpl.
      rewrite xDef.
...
Proof.
  ...
  - (* Core-Idem *)
    ...
    destruct cx; unfold pcore, flowintRAcore; destruct x;
      try (intros H1; inversion H1); try done.
  - (* Core-Mono *)
    intros x y cx.
    destruct cx; unfold pcore, flowintRAcore; destruct x; intros H1;
      intros H2; inversion H2; destruct y; try eauto.
...
End flowint.
  ...
  Context (flowdom: Type) `{CCM flowdom} `{EqDecision Node} `{Countable Node}.
  ...
  Proof.
    ...
    { pose proof (intComp_dom _ _ V12).
      ...
      unfold op in H1.
      rewrite Eq in H1.
      clear -H2 H1 Disj12 Disj13. set_solver. }
    rewrite I2_def.
    rewrite I3_def.
    f_equal.
    case_eq Ir2.
    ...
    destruct (decide _), (decide _).
      + destruct (decide (n ∈ dom I1)).
        * unfold dom, flowint_dom in e1.
          ...
          intros.
          rewrite H4 in H3.
          repeat rewrite (ccm_comm (out _ _)) in H3.
          apply ccm_cancel in H3.
          rewrite I2_def I3_def Ir2_def Ir3_def in H3.
          unfold out, out_map in H3.
          simpl in H3.
          by rewrite H3.
        * assert (n ∈ dom I2) by set_solver.
          assert (n ∈ dom I3) by set_solver.
          pose proof (intValid_in_dom_not_out I2 n V2 H1).
          pose proof (intValid_in_dom_not_out I3 n V3 H2).
          rewrite I2_def in H3.
          rewrite Ir2_def in H3.
          rewrite I3_def in H4.
          rewrite Ir3_def in H4.
          unfold out, out_map in H3, H4.
          simpl in H3, H4.
          rewrite H3.
          rewrite H4.
          reflexivity.
  ...
  Proof.
    unfold Cancelable.
    intros n I1 I2 V ?.
    pose proof (intComp_cancelable _ _ _ V H1).
    eauto.
  Qed.
  ...
  Lemma contextualLeq_insert_node_1' (Ip I1 In': flowintUR) (new_node tp n': Node) m1:
     let I_new := int {| infR := {[ new_node := inf Ip tp ]};
                       outR := <<[ tp := m1 ]>>∅ |} in 
    ...
    dom (out_map Ip) = {[n']} ->
    I1 = int {| infR := {[ tp := m1 ]}; outR := out_map Ip |} ->
    (forall I0, ✓ (I0 ⋅ Ip) /\ out (I0 ⋅ Ip) new_node = 0 /\ (out I0 tp = inf Ip tp) /\
             (dom I0 ## dom Ip) /\ (dom I0 ## dom I_new) /\ (dom I0 ## dom In') ->
           let I0':= int {| infR := inf_map I0; outR := <<[ tp := 0%CCM]>> <<[ new_node := out I0 tp ]>>(out_map I0) |}
           in contextualLeq (I0 ⋅ Ip) (I0' ⋅ I1 ⋅ I_new) /\
              inf (I0' ⋅ I1 ⋅ I_new) new_node = 0).
   Proof.
     intros ? Hdom_n'_In' HdomIp Hne_new_tp Hne_new_n' Hne_tp_n' Hdom_out_Ip_n' HI1 I0 I0p_cond I0'.
     ...
     assert (✓ (I1 ⋅ I_new)) as VI1_new.
     {
       ...
       * assert (i = tp) as ->.
         ...
         rewrite /inf /=.
         rewrite /I_new /out nzmap_lookup_total_insert lookup_insert /=.
     ...
     assert ( ✓ (I0' ⋅ (I1 ⋅ I_new))) as VI0'_I1_Inew.
     {
       ...
       + intros i x Hix.
         ...
         rewrite ccm_right_id.
         2:  { set_solver. }
         assert (out_map Ip !!! i = 0) as ->.
         { apply nzmap_elem_of_dom_total2. set_solver. }
         by rewrite ccm_comm ccm_right_id ccm_pinv_unit /inf Hix'.
       + intros i x Hix.
         rewrite /I0'. rewrite /out /=.
         destruct (decide (i ∈ dom I1 ∪ dom I_new)) as [Hx | Hy].
         {
           ...
           }
           {
             ...
             rewrite /out in HI0_p.
             rewrite HI0_p nzmap_lookup_total_insert_ne /=.
             rewrite nzmap_lookup_total_insert.
