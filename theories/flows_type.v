...
Section cmra.
    Context  `{CCM flowdom} (* `{EqDecision Node} *) `{Countable Node}  .
...
Canonical Structure flowintUR : ucmra := Ucmra flowintT flowint_ucmra_mixin.
  ...
  Proof.
    ...
    - unfold dom, flowint_dom in n_nin_I1.
      ...
      + assert (n ∈ dom I3) as n_in_I3.
        ...
        assert ((x2 - out (int Ir1) n)%CCM = (x3 - out (int Ir1) n)%CCM) by naive_solver.
  ...
  Check  out_map _ !!! _ = 0.
  Lemma replacement_theorem (Io In In' : flowintUR) :
    ✓ (In ⋅ Io) → 
      dom In' ∩ dom Io = ∅ → 
        (∀ (n : Node), n ∈ dom In' ∖ dom In → out_map Io !!! n = 0%CCM) →
          contextualLeq In In' →
  ...
  Lemma contextualLeq_add_empty_node (I: flowintUR) (n: Node) : 
    let In := int {| infR := {[ n := 0%CCM ]}; outR := ∅ |} in
    ✓ I → 
    n ∉ dom I → 
    out I n = 0%CCM →
  ...
  Proof.
    ...
    set In1': flowintUR := int {| infR := {[ n1 := 0%CCM ]}; outR := ∅ |}.
    ...
    { apply intValid_composable.
      ...
      - simpl. rewrite dom_insert_L.
        rewrite dom_empty. 
        destruct (decide (m = 0%CCM)) as [Hm | Hm].
  ...
  Lemma flowint_update : ∀ (Io I_n I_n': flowintUR),
    ...
    (dom I_n' ∩ dom Io = ∅) → 
    (∀ n, n ∈ dom I_n'∖dom I_n → out_map Io !!! n = 0%CCM) →
