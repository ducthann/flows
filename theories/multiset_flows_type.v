...
Require Export flows.flows flows.ccm.
...
Section multiset_flows.
  Context {flowdom : Type} `{CCM flowdom} `{Countable Node} `{Countable K}.
...
Proof.
  ...
  { rewrite H1. by rewrite lookup_total_lifting. }
  rewrite Hout nzmap_lookup_empty in H2.
  unfold ccmunit, ccm_unit in H2. simpl in H2.
  unfold nat_unit, ccmop, nat_op in H2.
  assert (out I2 n !!! k = 0). lia.
  rewrite H3. rewrite nzmap_lookup_empty. done. 
Qed. 
...
Proof.
  ...
  rewrite n1 in H2.
  rewrite lookup_total_lifting in H2.
  unfold ccmop, ccm_op in H2.
  unfold K_multiset_ccm,ccmunit,ccm_unit,nat_ccm,
    nat_unit,nat_op in kOut, not_k_out, H2.
  rewrite not_k_out in H2.  
  lia.
...
Proof.
  ...
  - intros. apply Hkb in e as HxB.
    rewrite nzmap_lookup_total_map_set in H1.
    unfold ccmunit, ccm_unit, nat_ccm, nat_unit in H1. simpl.
    assert (out I n !!! x - 1 = 0). lia.
    contradiction. done.
  - intros. set_solver.
  - intros. rewrite nzmap_lookup_total_map_set_ne in H1.
  ...
  - intros. rewrite nzmap_lookup_total_map_set_ne.
    rewrite elem_of_difference in H1 *; intros.
    destruct H1 as [H1 _].
    rewrite nzmap_elem_of_dom_total in H1 *; intros.
    unfold out. all: done.
Qed.    
  ...
  - replace n'.
    ...
    + destruct (inf_map I1' !! n); inversion Hin_eq.
      ...
      * replace I2'. simpl. rewrite lookup_insert.
        ...
        ** rewrite !nzmap_lookup_total_map_set.
           pose proof Hf k e0.
           unfold inf, out at 1 in H1.
           rewrite Hi2n in H1. simpl in H1.
           apply H1. all: done.
...
Proof.
  ...
  - rewrite elem_of_union.
    ...
    destruct H1.
    rewrite nzmap_lookup_total_map_set_ne;
    rewrite nzmap_elem_of_dom_total in H1 *; auto.
    contradiction.
Qed.
...
Proof.
  ...
  - intros. apply Hkb in e as HxB.
    rewrite nzmap_lookup_total_map_set in H1.
    unfold ccmunit, ccm_unit, nat_ccm, nat_unit in H1. 
...
Arguments multiset_flowint_ur _ _ _ {_ _} : assert.
Arguments inset _ _ _ {_ _} _ _ : assert.
Arguments outset _ _ _ {_ _} _ _ : assert.
Arguments in_inset _ _ _ {_ _} _ _ _ : assert.
Arguments in_outset _ _ _ {_ _} _ _ _ : assert.
Arguments in_outsets _ _ _ {_ _} _ _ : assert.
Arguments closed _ _ _ {_ _} _ : assert.
